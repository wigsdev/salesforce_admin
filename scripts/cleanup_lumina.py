...
def clean_garbage_data():
    ...
    try:
        ...
        garbage_query = db.query(LuminaDeliverable).filter(
            (LuminaDeliverable.title.op('~')('^[0-9]+-')) |  # Empieza con numeros y guion
            (LuminaDeliverable.reference.like('%/Gestor_de_Versiones/%')) | # Contiene rutas de archivo antiguas
            (LuminaDeliverable.reference.like('%.md')) # Es un archivo markdown literal
        )
