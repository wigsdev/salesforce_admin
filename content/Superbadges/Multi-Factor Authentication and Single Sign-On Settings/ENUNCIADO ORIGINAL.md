...
First, you need to make sure that users cannot log in to the org with their Salesforce credentials. Create a permission set called Single Sign-OnCopiar with an API name of Single_Sign_OnCopiar for this requirement, and use Murphy Jean as the SSO test user. Be sure to set a Federation ID for this user as you’ll need it in later steps. Use whatever value you’d like for Federation ID.
...
Issuer	https://axiomsso.herokuapp.comCopiar
...
Identity Provider Login URL	https://axiomsso.herokuapp.com/RequestSamlResponse.actionCopiar
...
The Break Glass user is required to pass a MFA verification challenge. Use a permission set labeled MFA Authorization for Break Glass AdminCopiar with the API Name MFA_Authorization_for_Break_Glass_AdminCopiar for this requirement.

Lightning Login
Enable Lightning Login to add an extra layer of security and to streamline the Break Glass admin login procedure. Make sure that only users with the permission set labeled Lightning Login UserCopiar (API Name Lightning_Login_UserCopiar) can log in with this method.
...
2
SAML Configurations and Testing
