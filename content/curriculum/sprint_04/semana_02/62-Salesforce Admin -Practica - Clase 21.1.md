...
Data Import y Manipulación de Masas
Modos de Operación: Insert, Update, Upsert y Delete. Explicación de por qué Upsert es la opción más segura en migraciones.
Mapeo Inteligente: Cómo la extensión identifica columnas por "Label" o "API Name".
Batch Size: Cuándo bajar el tamaño del lote (de 200 a 1) para debugear errores de triggers o validaciones específicas sin que falle todo el proceso.
Manejo de Errores: Cómo descargar solo los registros fallidos para corregirlos y re-subirlos.
...
LINK
EJERCICIO 2
Marcado de Cuentas de Prueba
Consigna: "Tomen 3 Cuentas de su Org, llévenlas a Excel, agreguen el sufijo -TEST a su nombre y actualícenlas en Salesforce usando únicamente el ID y el nuevo Nombre."
Pistas de Oro para el Challenge:
La Llave Maestra: Recordar que para un Update, el campo Id es obligatorio. Sin ID, Inspector no sabe a quién actualizar.
Cuidado con los Espacios: Si copian el nombre desde Salesforce a mano, asegúrense de no dejar espacios extras al final en el Excel.
Mapeo Manual: Si por alguna razón la columna del Excel se llama "Nombre de Cuenta" y no "Name", hay que seleccionar el campo correcto en el menú desplegable de mapeo de Inspector.
