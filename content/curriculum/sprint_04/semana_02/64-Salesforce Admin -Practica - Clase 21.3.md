...
Trucos de Expertos y Trampas Mortales
La Trampa del "Deleted Records": Cómo consultar la papelera de reciclaje marcando la casilla "Include Deleted" (SOQL ALL ROWS).
Queries Relacionales: Cómo traer datos del Padre en una sola query (ej: SELECT Name, Account.Name FROM Contact).
Excel Power: Truco de copiar/pegar directamente desde Excel a Inspector sin guardar archivos CSV.
Límites de Seguridad: Recordar que Inspector respeta el Perfil/Permission Set del usuario. Si no lo ves en el CRM, no lo ves en Inspector (a menos que sea un campo de sistema).



LINK
EJERCICIO 3
Query Relacional: Ejecutar una query que traiga Oportunidades con el nombre de su Cuenta y el Owner.
Restauración: Localizar un registro borrado recientemente mediante una query de "Deleted Records".
Limpieza: Simular un borrado masivo (Delete) de registros de prueba (ej: Leads de prueba).
Ejercicio de Clase (5 min): 
 "Escribe una query que devuelva el Nombre de un Contacto y el Nombre de la Cuenta a la que pertenece. Screen de la query final."
...
Verificación de HU
Preguntas y dudas
Asignación y dificultad
Redacción y columnas

HOY

QUÉ HACEMOS HOY?
1-Refinamiento de HU
2-Trails atrasados
