...
**HUs Relacionadas**: [HU-010](../../Archivos_intermedios/HISTORIAS_DE_USUARIO_ES.md) (Visibilidad), [HU-011](../../Archivos_intermedios/HISTORIAS_DE_USUARIO_ES.md) (MFA), [HU-012](../../Archivos_intermedios/HISTORIAS_DE_USUARIO_ES.md) (FLS)
...
2.  Baja hasta **Materia Sharing Rules**.
3.  **New**.
4.  Label: `Compartir_con_Dueno_Comision`.
5.  Rule Type: **Based on Criteria**.
6.  Criteria: `Activa` equals `True`.
...
2.  **Prueba de Visibilidad**: Ve a **Materias**.
    *   **Esperado**: Solo debe ver "Pociones" (suya). NO debe ver materias de otros.
3.  **Prueba de Edición**: Entra a una **Inscripción** de "Pociones".
    *   **Esperado**: Puede editar `Nota Final`. ✅
4.  **Prueba de Datos Sensibles**: Ve al **Alumno** relacionado.
    *   **Esperado**: NO ve `DNI` ni `Teléfono`. 🙈

#### 🎭 Escenario B: Integridad Administrativa (Dolores)
1.  Haz **Login** como Dolores.
2.  **Prueba de Creación**: Crea una nueva **Inscripción**.
    *   **Esperado**: Éxito. ✅
3.  **Prueba Read-Only**: Intenta modificar una `Nota Final` existente.
    *   **Esperado**: Campo bloqueado/grisado. ⛔

#### 🎭 Escenario C: Auditoría (Director/Admin)
1.  Loguéate como Admin.
2.  Ve a una Inscripción modificada por Severus.
3.  Revisa **Field History**.
    *   **Esperado**: "Changed Nota Final from X to Y by Severus Snape". 🕵️‍♂️
