...
#### 🏷️ [HU-010] Matriz de Visibilidad (Privacidad)
*   **Prioridad**: Alta (Compliance).
*   **As a** (Como): Responsable de Privacidad de Datos (DPO).
*   **I Want** (Quiero): Que los legajos de los estudiantes sean privados por defecto.
...
*   **✅ Acceptance Criteria**:
    ...
    - [ ] El permiso está centralizado en un Permission Set (`Lumina_MFA_Access`).
...
*   **✅ Acceptance Criteria**:
    - [ ] **Bedelía**: Puede editar `Email`, `Address` del alumno, pero campo `Final_Grade__c` está grisado (Read-Only).
    - [ ] **Profesor**: Puede editar campo `Final_Grade__c`, pero no puede cambiar el `National_ID` del alumno.
