...
*Si no ves Alumnos/Carreras/etc en la lista, es porque no creamos sus "Tabs" al definir los objetos.*
...
5.  **Object**: Selecciona `Alumno`. **Tab Style**: Elige cualquiera (ej: "People"). **Next**.
6.  **Profiles**: "Apply one tab visibility to all profiles" > **Default On**. **Next**.
7.  **Apps**: Desmarca "Include Tab". **Save**.
8.  **Repite** para `Carrera`, `Materia`, `Inscripción`, `Asistencia` y `Nota`.
