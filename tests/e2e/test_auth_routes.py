...
def test_login_successful(mock_auth):
    ...
    response = client.post(
        "/api/auth/login",
        json={"email": "test@lumina.edu", "password": "correctpassword"},
    )
...
def test_login_failed(mock_auth):
    ...
    response = client.post(
        "/api/auth/login",
        json={"email": "wrong@lumina.edu", "password": "wrongpassword"},
    )
...
def test_get_current_user_me_overriding_dependency():
    ...
    try:
        response = client.get(
            "/api/users/me", headers={"Authorization": "Bearer mocked-token"}
        )
